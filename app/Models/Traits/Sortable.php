...
trait Sortable
{
    ...
    {
        $request_params = array_filter(request()->all(),function($value) {
            return ($value !== null && $value !== false && $value !== '');
        });
        //dd($request_params,array_key_exists("sort",$request_params));
        if (array_key_exists("sort",$request_params)) { // allFilled() is macro
            return $this->queryOrderBuilder($query, request()->only(['sort']));
        }
    ...
    private function queryOrderBuilder($query, array $sortParameters)
    {
        ...
        if(Str::startsWith($sortParameters["sort"],"-")){
            $direction = "DESC";
        ...
        if (isset($model->sortable) && in_array($column, $model->sortable)) {
